...
def _new_count_entry() -> dict[str, Any]:
...
def compute_cas_stats(cas, excluded_types: set[str]) -> tuple[dict[str, Any], set[str]]:
...
def _merge_counts(counts_list: list[dict[str, Any]]) -> dict[str, Any]:
...
def _average_counts(counts_list: list[dict[str, Any]]) -> dict[str, Any]:
    if not counts_list:
        return {}
...
def _max_counts(counts_list: list[dict[str, Any]]) -> dict[str, Any]:
    best_counts = {}
    best_total = -1
...
def _aggregate_document_counts(
    counts_list: list[dict[str, Any]], aggregation_mode: str
) -> dict[str, Any]:
...
def summarize_document_categories(project_documents: list[dict[str, Any]]) -> dict[str, int]:
...
def summarize_token_categories(
    project_documents: list[dict[str, Any]], type_counts: dict[str, dict[str, Any]]
) -> dict[str, int]:
    categories = {state: 0 for state in DOCUMENT_STATES}
...
def get_curated_document_names(project_documents: list[dict[str, Any]]) -> set[str]:
    return {
        document["name"]
        for document in project_documents
...
def filter_type_counts_to_documents(
    type_counts: dict[str, dict[str, Any]], allowed_documents: set[str]
) -> dict[str, dict[str, Any]]:
    filtered_counts = {}
...
def build_exported_type_counts(
    type_counts: dict[str, dict[str, Any]], project_documents: list[dict[str, Any]]
) -> dict[str, dict[str, Any]]:
    exported_counts = {}
    document_states = {
